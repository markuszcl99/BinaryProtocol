...
public class DeserializeTest {
    ...
    /**
     * 测试protostuff反序列化 使用自己写的protostuff序列化工具
     *
     * @param protostuffFromUtilState
     * @return
     * @throws Exception
     */
    @Benchmark
    public User protostuffDeSerializeFromUtil(ProtostuffFromUtilState protostuffFromUtilState) throws Exception {
        User user = ProtostuffUtils.deserialize(protostuffFromUtilState.bytes, User.class);
        assert "zzs0".equals(user.getName());
        return user;
    }

    @State(Scope.Benchmark)
    public static class ProtostuffFromUtilState {

        byte[] bytes;

        @Setup(Level.Trial)
        public void prepare() throws Exception {
            bytes = ProtostuffUtils.serialize(new UserService().get());
            System.err.println("protostuff from utils serialized data size:" + bytes.length);
        }
     ...
     */
    @Benchmark
    public UserProto.User protobufDeSerialize(ProtobufState protobufState) throws Exception {
        UserProto.User user = UserProto.User.parseFrom(protobufState.bytes);
        assert "zzs0".equals(user.getName());
        return user;
    }
    ...
    public static class ProtobufState {
        ...
        @Setup(Level.Trial)
        public void prepare() throws Exception {
            bytes = new UserService().getProto().toByteArray();
            System.err.println("protobuf serialized data size:" + bytes.length);
        }

    }

    @Benchmark
    public User xmlDeSerialize(XmlState xmlState) throws Exception {
        User user = new User();
        XmlUtils.deserialize(xmlState.data, user);
        assert "zzs0".equals(user.getName());
        return user;
    }
    ...
    public static class XmlState {
        ...
        @Setup(Level.Trial)
        public void prepare() throws Exception {
            data = XmlUtils.serialize(new UserService().get());
            System.err.println("protobuf serialized data size:" + data.getBytes(StandardCharsets.UTF_8).length);
        }
    ...
    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(DeserializeTest.class.getSimpleName())
                .forks(1)
                .threads(1)
                .warmupIterations(10)
                .measurementIterations(10)
                .result("DeserializeTest-V4.json")
